...
class DbController extends ApiController
{
    ...
    /**
     * Batch endpoint: process multiple /api/db queries in a single HTTP request.
     * Accepts {"queries": [{table, action, columns, filters, order, limit, offset}, ...]}
     * Returns {"results": [{data: [...], error: null}, ...]}
    ...
    public function handleBatch(Request $request)
    {
        ...
        if (!is_array($queries) || empty($queries)) {
            return response()->json(['error' => 'queries array is required'], 400);
        }

        if (count($queries) > 10) {
            return response()->json(['error' => 'Max 10 queries per batch'], 400);
        }

        $results = [];

        foreach ($queries as $index => $queryDef) {
            try {
                // Create a sub-request for each query
                $subRequest = Request::create(
                    ...
                    [],
                    $request->server->all(),
                    json_encode($queryDef)
                );

                // Copy auth from parent request
                ...
                if ($request->hasHeader('X-Tenant')) {
                    $subRequest->headers->set('X-Tenant', $request->header('X-Tenant'));
                }

                // Merge input manually for Laravel to pick up
                $subRequest->merge($queryDef);

                $response = $this->handle($subRequest);
                $responseData = json_decode($response->getContent(), true);

                $results[] = [
                    'data' => $responseData['data'] ?? null,
                    'error' => $responseData['error'] ?? null,
                    'count' => $responseData['count'] ?? null,
                ];
            } catch (\Throwable $e) {
                $results[] = [
                    'data' => null,
    ...
    private function applyDefaultAcademicSelectScope(string $table, $query, $filters, ?string $tenantId): void
    {
        ...
        $this->whereCurrentAcademicPeriodOrLegacy($query, $period);
    }
    ...
    private function whereCurrentAcademicPeriodOrLegacy($query, array $period): void
    {
        $tahunAjaran = (string) ($period['tahun_ajaran'] ?? '');
        $semester = (string) ($period['semester'] ?? '');

        $query->where(function ($periodQuery) use ($tahunAjaran, $semester) {
            $periodQuery
                ->where(function ($current) use ($tahunAjaran, $semester) {
                    $current
                ...
                ->orWhere(function ($legacy) {
                    $legacy
                        ->where(function ($year) {
                            $year->whereNull('tahun_ajaran')->orWhere('tahun_ajaran', '');
                        })
                        ->where(function ($semesterQuery) {
                            $semesterQuery->whereNull('semester')->orWhere('semester', '');
                        });
                });
        });
    }
