...
class JadwalController extends ApiController
{
    public function index(Request $request)
    {
        ...
        if ($kelasId = $request->query('kelas_id')) $query->where('kelas_id', $kelasId);
        if ($guruId = $request->query('guru_id')) $query->where('guru_id', $guruId);
        if ($hari = $request->query('hari')) $query->where('hari', $hari);
