...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        foreach ([
            'profiles_tenant_rfid_uid_normalized_idx',
            'jadwal_tenant_kelas_hari_time_idx',
            'absensi_rfid_lookup_idx',
            'rfid_devices_lower_device_idx',
            'rfid_device_events_tenant_device_event_lookup_idx',
            'rfid_scans_tenant_created_idx',
            'settings_tenant_id_idx',
            'absensi_rfid_settings_tenant_idx',
        ] as $index) {
    ...
    {
        if ($this->hasColumns('profiles', ['tenant_id', 'rfid_uid'])) {
            $where = Schema::hasColumn('profiles', 'deleted_at')
                ? 'WHERE rfid_uid IS NOT NULL AND deleted_at IS NULL'
                : 'WHERE rfid_uid IS NOT NULL';

            DB::statement(<<<SQL
CREATE INDEX IF NOT EXISTS profiles_tenant_rfid_uid_normalized_idx
ON profiles (
    tenant_id,
    upper(regexp_replace(coalesce(rfid_uid, ''), '\\s+', '', 'g'))
)
{$where}
SQL);
        }

        if ($this->hasColumns('jadwal', ['tenant_id', 'kelas_id', 'hari', 'jam_mulai', 'jam_selesai'])) {
            DB::statement(<<<'SQL'
CREATE INDEX IF NOT EXISTS jadwal_tenant_kelas_hari_time_idx
ON jadwal (
    tenant_id,
    lower(kelas_id),
    lower(hari),
...
SQL);
        ...
        if ($this->hasColumns('absensi', ['tenant_id', 'uid', 'tanggal', 'mapel', 'kelas'])) {
            DB::statement(<<<'SQL'
CREATE INDEX IF NOT EXISTS absensi_rfid_lookup_idx
ON absensi (tenant_id, uid, tanggal, mapel, kelas)
SQL);
        }

        if ($this->hasColumns('rfid_devices', ['device_id'])) {
            DB::statement(<<<'SQL'
CREATE INDEX IF NOT EXISTS rfid_devices_lower_device_idx
ON rfid_devices (lower(device_id))
SQL);
        }

        if ($this->hasColumns('rfid_device_events', ['tenant_id', 'device_id', 'event_id'])) {
            DB::statement(<<<'SQL'
CREATE INDEX IF NOT EXISTS rfid_device_events_tenant_device_event_lookup_idx
ON rfid_device_events (tenant_id, lower(device_id), event_id)
WHERE event_id IS NOT NULL
SQL);
        }

        if ($this->hasColumns('rfid_scans', ['tenant_id', 'created_at'])) {
            DB::statement(<<<'SQL'
CREATE INDEX IF NOT EXISTS rfid_scans_tenant_created_idx
ON rfid_scans (tenant_id, created_at DESC)
SQL);
        }

        if ($this->hasColumns('settings', ['tenant_id'])) {
            DB::statement('CREATE INDEX IF NOT EXISTS settings_tenant_id_idx ON settings (tenant_id)');
        }

        if ($this->hasColumns('absensi_rfid_settings', ['tenant_id'])) {
            DB::statement('CREATE INDEX IF NOT EXISTS absensi_rfid_settings_tenant_idx ON absensi_rfid_settings (tenant_id)');
        }
    }

    private function replaceRfidFunction(): void
    {
        DB::unprepared(<<<'SQL'
...
BEGIN
  ...
  IF v_manual_enabled THEN
    ...
    ON CONFLICT (tenant_id, kelas, tanggal, uid, mapel) DO NOTHING
