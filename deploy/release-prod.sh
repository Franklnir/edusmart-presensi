...
Options:
  ...
  --backend-image <image> Override EDUSMART_BACKEND_IMAGE untuk release ini
  --nginx-image <image>   Override EDUSMART_NGINX_IMAGE untuk release ini
  -h, --help              Tampilkan bantuan

Contoh:
  deploy/release-prod.sh --ref v1.5.0
  deploy/release-prod.sh --ref main --pull-images
  deploy/release-prod.sh --ref v1.5.0 --pull-images \
    --backend-image ghcr.io/owner/repo/backend:v1.5.0 \
    --nginx-image ghcr.io/owner/repo/nginx:v1.5.0
USAGE
}
...
if [[ "$PULL_IMAGES" == "true" ]]; then
  echo "[4/8] Pull image prebuilt & deploy tanpa build di VPS..."
