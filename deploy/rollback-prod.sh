...
Options:
  ...
  --skip-build            Lewati rebuild image, hanya restart service dari image lokal
  --pull-images           Pull image prebuilt dari registry lalu deploy tanpa build di VPS
  --backend-image <image> Override EDUSMART_BACKEND_IMAGE untuk rollback ini
  --nginx-image <image>   Override EDUSMART_NGINX_IMAGE untuk rollback ini
  -h, --help              Tampilkan bantuan

Contoh:
  deploy/rollback-prod.sh --ref v1.4.2
  deploy/rollback-prod.sh --ref v1.4.2 --pull-images \
    --backend-image ghcr.io/owner/repo/backend:v1.4.2 \
    --nginx-image ghcr.io/owner/repo/nginx:v1.4.2
...
if [[ -n "$BACKEND_IMAGE_OVERRIDE" ]]; then
  export EDUSMART_BACKEND_IMAGE="$BACKEND_IMAGE_OVERRIDE"
fi
if [[ -n "$NGINX_IMAGE_OVERRIDE" ]]; then
  export EDUSMART_NGINX_IMAGE="$NGINX_IMAGE_OVERRIDE"
fi

APP_SERVICES=(backend nginx worker scheduler rfid_bridge caddy mosquitto_reloader evolution_api)
OWN_IMAGE_SERVICES=(backend nginx)
...
if [[ "$PULL_IMAGES" == "true" ]]; then
  echo "[4/7] Pull image prebuilt & deploy tanpa build di VPS..."
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" pull "${OWN_IMAGE_SERVICES[@]}"
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" up -d --no-build \
    "${APP_SERVICES[@]}"
elif [[ "$SKIP_BUILD" == "true" ]]; then
  echo "[4/7] Restart service produksi tanpa rebuild..."
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" up -d --no-build \
    "${APP_SERVICES[@]}"
else
