...
|---|---|---|---|---|---|
| Classes | `src/pages/admin/Kelas.jsx` | CRUD kelas plus schedule preview/options | `/api/db` (`kelas`, `jadwal`) | `/api/v2/classes`; schedule V2 pending | PARTIAL |
| Teachers | `src/pages/admin/Guru.jsx` | list/update teacher and teaching-schedule dependency | `/api/db` (`profiles`, `jadwal`) | `/api/v2/teachers`; schedule V2 pending | PARTIAL |
...
| Attendance | `src/pages/admin/Scan.jsx` | schedule lookup, attendance upsert, scan status | `/api/db` (`jadwal`, `absensi`, `rfid_scans`) | `/api/v2/attendance`; schedules/RFID V2 pending | PARTIAL |
| Attendance | `src/pages/guru/AbsensiGuru.jsx` | schedule/student/attendance/request reads and mutations | `/api/db` | `/api/v2/attendance`, `/api/v2/attendance-requests`; schedules V2 pending | PARTIAL |
| Attendance | `src/features/attendance/hooks/useStudentAttendanceData.js` | today/week schedule and attendance status | `/api/db` (`jadwal`, attendance tables) | attendance V2; schedules V2 pending | PARTIAL |
...
| Assignments | `src/pages/guru/TugasGuru.jsx` | list/detail/CRUD, publish-like state, submission list/grade, schedule/class options, file upload/delete | `/api/tugas*`, `/api/db`, `/api/storage/*` | `/api/v2/assignments`, `/api/v2/submissions`; Upload/Attachment and schedules pending | PARTIAL |
| Assignments | `src/pages/siswa/Tugas.jsx` | list/detail/submit/revise, schedule read, file upload/delete | `/api/tugas*`, `/api/db`, `/api/storage/*` | assignments/submissions V2; Upload/Attachment and schedules pending | PARTIAL |
...
| Upload/Attachment | `src/lib/supabase.js` storage compatibility adapter | initiate, relay upload, complete, signed/public URL, download, delete | `/api/storage/*` | `/api/v2/uploads`, `/api/v2/attachments/*` incomplete | PARTIAL |
| Upload/Attachment | `src/pages/guru/TugasGuru.jsx`, `src/pages/siswa/Tugas.jsx` | assignment/submission upload progress and removal | `/api/storage/*` | Upload/Attachment V2 pending production provider | BLOCKED |
...
| Grades | `src/pages/guru/Laporan.jsx` | weights, manual grades, task/quiz recap, export inputs | `/api/db`, `/api/reports/*` | `/api/v2/grades*` pending | LEGACY |
| Grades/Raport | `src/pages/guru/RapotSiswa.jsx` | raport CRUD, items, schedule-derived subjects | `/api/db` (`rapot_siswa*`, `jadwal`) | grade/raport V2 pending schema mapping | LEGACY |
...
| Quiz authoring | `src/pages/guru/Quiz.jsx` | dashboard/detail, CRUD quiz/questions/options, schedule, publish/close/clone, essay grade/retake | `/api/quiz/*`, `/api/db`, `/api/storage/*` | `/api/v2/quizzes*`, question/attempt V2 pending | LEGACY |
| Quiz attempts | `src/pages/siswa/Quiz.jsx` | dashboard/detail/start/autosave/submit/violation/retake/media | `/api/quiz/*`, `/api/storage/object` | `/api/v2/quiz-attempts*` and Attachment download pending | LEGACY |
...
| Schedules | `src/pages/admin/Kelas.jsx` | schedule list/create/update/delete/copy/preview | `/api/db` (`jadwal`) | `/api/v2/schedules*` pending | LEGACY |
| Schedules | `src/pages/guru/JadwalGuru.jsx` | teacher schedule, student/extracurricular schedule views | `/api/db` (`jadwal`, related tables) | `/api/v2/schedules*` pending | LEGACY |
| Schedules | `src/pages/admin/Guru.jsx`, `src/pages/admin/Scan.jsx`, `src/pages/guru/AbsensiGuru.jsx` | assignment/attendance scope and options | `/api/db` (`jadwal`) | `/api/v2/schedules*` pending | LEGACY |
| Schedules | `src/pages/guru/TugasGuru.jsx`, `src/pages/guru/Quiz.jsx`, `src/pages/guru/Laporan.jsx`, `src/pages/guru/RapotSiswa.jsx` | class/subject options and report scope | `/api/db` (`jadwal`) | `/api/v2/schedules*` pending | LEGACY |
| Schedules | `src/pages/siswa/Tugas.jsx`, `src/pages/siswa/Quiz.jsx`, attendance data hooks, `MapelOptions.jsx` | student calendar, active-session and subject options | `/api/db` (`jadwal`) | `/api/v2/schedules*` pending | LEGACY |
...
| Monitor UI | `src/pages/admin/MonitorLog.jsx`, `SuperMonitorLog.jsx` | filter placeholder and legacy endpoint observation | observes `/api/db`; does not call it for business data | `/api/v2/frontend-logs*` | MIGRATED |
