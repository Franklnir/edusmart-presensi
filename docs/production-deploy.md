...
- Untuk VPS RAM kecil, alur yang lebih direkomendasikan adalah build image di GitHub Actions/GHCR lalu deploy dengan `deploy/release-prod.sh --pull-images`, sehingga VPS tidak menjalankan build.

Contoh deploy dari image GHCR prebuilt:
...
Detail alur push aman, GHCR, dan deploy tanpa build ada di `docs/github-ghcr-vps-deploy.md`.
...
- Push aman + GHCR + deploy VPS tanpa build:
  - `docs/github-ghcr-vps-deploy.md`
