...
/**
 * Batch multiple /api/db queries into a single HTTP request.
 * @param {Array<{table, action?, columns?, filters?, order?, limit?, offset?}>} queries
 * @returns {Promise<Array<{data, error}>>}
 */
export const apiBatchFetch = async (queries) => {
  if (!Array.isArray(queries) || queries.length === 0) return []

  // If only one query, use regular apiFetch for simplicity
  if (queries.length === 1) {
    const res = await apiFetch('/api/db', {
      method: 'POST',
      body: {
        ...queries[0],
        action: queries[0].action || 'select'
      }
    })
    return [res]
  }
  ...
  const res = await apiFetch('/api/db/batch', {
    method: 'POST',
    body: { queries: normalizedQueries }
  })
  ...
  const results = res.data?.results || res.raw?.results || []
  return results.map((r) => ({
    data: r.data ?? null,
    error: r.error ? makeError(r.error, 500) : null,
    raw: r
  }))
}
