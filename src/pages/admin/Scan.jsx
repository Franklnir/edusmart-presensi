...
export default function Scan() {
  ...
  const handleProcessScan = useCallback(
    async (code, options = {}) => {
      if (!code) return
      const scanSource = options.source || (options.fromRealtime ? 'device' : 'web_admin')
      ...
      if (!scanOperationalActive) {
        ...
        try {
          ...
          cleanedUid = scanSource === 'web_nfc'
            ? normalizeBrowserNfcUid(cleanedUid)
            : String(cleanedUid).trim()
      ...
      const cleanedUid = scanSource === 'web_nfc'
        ? normalizeBrowserNfcUid(targetUid)
        ...
        if (fromRealtime && scanRowId) {
          ...
          const { error: errInsert } = await supabase
            ...
            .insert({
              card_uid: cleanedUid,
              status: 'processed',
              device_id: scanSource === 'web_nfc' ? 'WEB_NFC_BROWSER' : 'WEB_ADMIN_MANUAL'
            })
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-2">
                    <div className={`rounded-xl border p-4 ${browserNfcStatusMeta.card}`}>
                      ...
                      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <div className="rounded-lg bg-white px-3 py-2">
                          <p className="text-[11px] font-semibold uppercase text-gray-500">Scan dari HP</p>
                          <p className="mt-1 text-sm font-semibold text-gray-900">
                            {browserNfcReadCount} kartu terbaca
                          </p>
                        </div>
                      </div>
