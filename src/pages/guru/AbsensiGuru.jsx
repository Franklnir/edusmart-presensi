...
const RealTimeClock = () => {
  ...
  return (
    <div className="text-sm font-medium text-gray-700 bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm flex items-center gap-2">
...
const JadwalHariIniCard = ({ jadwal, currentTimeMinutes, onClick }) => {
  ...
  return (
    <div 
      className={`p-3 rounded-lg border cursor-pointer transition-all duration-200 hover:shadow-md ${config.color}`}
      onClick={onClick}
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-bold text-gray-900 text-xs">{jadwal.mapel}</h3>
        ...
        <div className="w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center ml-2 flex-shrink-0 shadow-sm">
...
const JadwalCard = ({ jadwal, onSelect, isSelected = false }) => {
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <h3 className="font-bold text-gray-900 mb-1 text-xs">{jadwal.mapel}</h3>
          <div className="space-y-1 text-[10px] text-gray-600">
            <div className="flex items-center gap-1">
              <span className="font-semibold text-gray-700">Kelas:</span>
              <span className="bg-gray-100 text-gray-800 px-1.5 py-0.5 rounded text-[10px] font-medium">
...
function AbsensiGuru() {
  ...
  const { myKelasList, schedulesForSelectedClass, jadwalByHari, jadwalHariIni } = useMemo(() => {
    if (!user?.id || !jadwalAll.length) return { 
      myKelasList: [], 
    ...
    try {
      ...
      if (updates.length > 0) {
        const { error: upsertError } = await supabase
          .from('absensi')
          .upsert(updates, { onConflict: 'kelas,tanggal,mapel,uid' })
      ...
      try {
        const [absensiRes, ajuanRes, siswaRes] = await Promise.all([
          supabase.from('absensi').select('*')
            .eq('kelas', kelas).eq('tanggal', tgl).eq('mapel', currentSchedule.mapel),
          supabase.from('absensi_ajuan').select('*')
            .eq('kelas', kelas).eq('tanggal', tgl).eq('mapel', currentSchedule.mapel),
          supabase.from('profiles').select('*')
            .eq('role', 'siswa').eq('kelas', kelas).order('nama')
        ])
        ...
        const siswaBelumAbsen = siswa.filter(s => 
          !absensi.find(a => a.uid === s.id) && 
          !ajuan.find(a => a.uid === s.id)
        )
  ...
  /* ===== Enhanced Jam Kosong Functions dengan Tanggal ===== */
  ...
  /* ===== Enhanced Real-time RFID Listeners ===== */
  useEffect(() => {
    if (!kelas || !currentSchedule?.mapel || !siswa.length || absenMode !== 'otomatis' || tgl !== getToday()) return
  ...
  const renderSiswaTable = (list, type) => {
    ...
    const getSourceInfo = (s) => {
      ...
      if (text.includes('rfid') || oleh.includes('rfid')) {
        return <span className="flex items-center gap-1 font-bold text-blue-600 text-xs"><span className="text-[10px]">📡</span> RFID</span>
      }
      if (text.includes('manual') || oleh === user?.id) {
        return <span className="flex items-center gap-1 font-bold text-gray-600 text-xs"><span className="text-[10px]">👨‍🏫</span> Manual Guru</span>
      }
      if (text.includes('auto') || oleh === 'system') {
        return <span className="flex items-center gap-1 font-bold text-orange-600 text-xs"><span className="text-[10px]">🤖</span> System</span>
      }
      return <span className="text-gray-600 text-xs">{s.komentar}</span>
    }
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
          <table className="w-full text-sm text-left border-collapse">
            <thead className="bg-gray-100 text-gray-700 font-bold uppercase text-xs">
              <tr>
                <th className="px-4 py-3 w-12 text-center border-b">No</th>
                <th className="px-4 py-3 w-16 text-center border-b">Foto</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {list.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-4 py-6 text-center text-gray-400 italic bg-white">
                ...
                list.map((s, index) => (
                  <tr key={s.id} className={`transition-colors ${rowColors[type]}`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      ...
                      <div 
                        className={`w-9 h-9 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-xs border border-blue-200 mx-auto ${s.photo_url ? 'hidden' : ''}`}
                      >
                        {initials(s.nama)}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col items-start gap-1">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wide ${badgeColors[type]}`}>
                          {type}
                        </span>
                        {getSourceInfo(s)}
                        {s.waktu && type === 'Hadir' && (
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          <div className="text-center md:text-left">
            <h1 className="text-2xl font-bold text-gray-800">Absensi Guru</h1>
            <p className="text-gray-500 text-sm">Sistem Manajemen Kehadiran Siswa Real-time</p>
          </div>
          <div className="flex flex-col items-center md:items-end gap-2">
            <RealTimeClock />
            <div className="flex gap-2">
              <span className={`px-2 py-1 rounded text-[10px] font-bold border uppercase tracking-wider ${isOnline ? 'bg-green-100 text-green-700 border-green-300' : 'bg-red-100 text-red-700 border-red-300'}`}>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          <div className="p-5 bg-gradient-to-b from-gray-50 to-white border-b border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wide">Pilih Kelas</label>
                <select 
                  className="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white shadow-sm" 
                  value={kelas} 
                  onChange={e => {
                ...
                >
                  ...
                  {myKelasList.map(k => (
                    <option key={k} value={k}>{formatKelasDisplay(k)}</option>
                  ))}
                </select>
              </div>
              {view === 'absen' && (
                <>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wide">Pilih Jadwal</label>
                    <select 
                      className="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white shadow-sm" 
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wide">Tanggal Absen</label>
                    <div className="flex gap-2">
                      <input 
                        type="date" 
                        className="flex-1 px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:ring-2 focus:ring-blue-500 shadow-sm" 
                        value={tgl} 
                        onChange={e => setTgl(e.target.value)} 
                      />
                      <button 
                        onClick={() => setTgl(getToday())} 
                        className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold shadow-sm transition-colors"
                      >
                        Hari Ini
                      </button>
                    </div>
          ...
          {view === 'absen' && currentSchedule && (
            <div className="bg-white border-b border-gray-200 px-6 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
              {/* Left: Schedule Info */}
              <div className="flex items-center gap-3 w-full md:w-auto">
                <div className={`p-2 rounded-lg flex-shrink-0 ${isAbsenOpen ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <div>
                  <div className="font-bold text-gray-800 text-sm">{currentSchedule.mapel}</div>
                  <div className="text-xs text-gray-500 flex items-center gap-2">
                    <span>{currentSchedule.jam_mulai} - {currentSchedule.jam_selesai}</span>
                    <span>•</span>
            ...
            <div className="flex gap-8 overflow-x-auto">
              {[
                ...
                <button 
                  ...
                  className={`py-4 text-sm font-bold border-b-2 flex items-center gap-2 transition-all ${view === tab.key ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                >
          ...
          <div className="p-6 bg-white min-h-[400px]">
            ...
            {view === 'absen' && (
              <div className="space-y-8">
                ...
                {ajuan.length > 0 && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 shadow-sm animate-pulse-slow">
                    <h4 className="font-bold text-amber-800 mb-3 flex items-center gap-2">
                      📨 Ajuan Izin Masuk <span className="bg-amber-200 text-amber-900 px-2 py-0.5 rounded-full text-xs">{ajuan.length}</span>
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {ajuan.map(a => (
                        <div key={a.id} className="bg-white p-3 rounded-lg border border-amber-100 flex justify-between items-center shadow-sm">
                          <div>
                ...
                {isLoading ? (
                  <div className="text-center py-20">
                    ...
                    <p className="text-gray-400 text-sm">Sedang memuat data...</p>
                  </div>
                ) : siswa.length === 0 ? (
                  <div className="text-center py-20 text-gray-400 bg-gray-50 rounded-xl border-2 border-dashed">
                    <div className="text-4xl mb-2">🎓</div>
                    <p>Silakan pilih <b>Kelas</b> dan <b>Jadwal</b> terlebih dahulu.</p>
                  </div>
                ) : (
                  <>
            ...
            {view === 'jadwal' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {Object.keys(jadwalByHari).length === 0 && (
                  <div className="col-span-3 text-center text-gray-400 py-20 italic">
                    Belum ada jadwal mengajar.
                  </div>
                )}
                {Object.entries(jadwalByHari).map(([hari, items]) => (
                  <div key={hari} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    <div className="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-5 py-3 font-bold text-sm tracking-wide uppercase">{hari}</div>
            ...
            {view === 'jam_kosong' && (
              <div className="space-y-8">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Jadwal Hari Ini</label>
                        <select 
                      ...
                      <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Guru Pengganti (Opsional)</label>
                        <select 
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white" 
                  ...
                  <div className="mt-6">
                    <button 
                      ...
                      className="w-full py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed shadow-lg transition-all"
                    >
                      {loadingJamKosong ? (
                ...
                {/* Riwayat Jam Kosong dengan Tanggal */}
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button 
                      ...
                      className="text-blue-600 text-sm font-bold hover:bg-blue-50 px-3 py-1 rounded transition-colors flex items-center gap-1"
                    >
                      🔄 Refresh
                    </button>
                  </div>
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                      <table className="w-full text-sm">
                        <thead className="bg-gray-100 text-gray-700 uppercase text-xs">
                          <tr>
                            <th className="p-3 text-left font-bold border-b">Tanggal</th>
                            <th className="p-3 text-left font-bold border-b">Mapel / Jam</th>
                            <th className="p-3 text-left font-bold border-b">Kelas</th>
                            <th className="p-3 text-left font-bold border-b">Alasan</th>
                            <th className="p-3 text-left font-bold border-b">Pengganti</th>
                            <th className="p-3 text-center font-bold border-b">Aksi</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                          {riwayatJamKosong.map(r => (
                            <tr key={r.id} className="hover:bg-gray-50">
                              <td className="p-3">
                                <div className="font-bold text-gray-800 text-sm">{formatDateDisplay(r.tanggal)}</div>
                                <div className="text-xs text-gray-500">{getDayName(r.tanggal)}</div>
                              </td>
                              <td className="p-3">
                                <div className="font-bold text-gray-800 text-sm">{r.mapel}</div>
                                <div className="text-xs text-gray-500">{r.jam_mulai} - {r.jam_selesai}</div>
                              </td>
                              <td className="p-3 text-gray-600 font-medium">{formatKelasDisplay(r.kelas)}</td>
                              <td className="p-3 text-gray-600 italic">{r.alasan}</td>
                              <td className="p-3 text-gray-600">{r.guru_pengganti || '-'}</td>
      ...
      {isIzinModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-[99] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl transform transition-all">
            <h3 className="font-bold text-lg mb-2 text-gray-800">📝 Beri Izin Manual</h3>
            <p className="text-sm text-gray-500 mb-4">Masukkan alasan kenapa siswa ini izin.</p>
            <textarea 
              className="w-full border border-gray-300 rounded-xl p-3 mb-5 focus:ring-2 focus:ring-blue-500 focus:outline-none" 
              rows="3" 
            ...
            <div className="flex justify-end gap-3">
              ...
              <button 
                ...
                className="px-5 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg transition-colors"
              >
                Simpan Izin
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[99] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center gap-3 mb-4 border-b pb-4">
              ...
              <div>
                <h3 className="font-bold text-lg text-gray-800">Detail Ajuan Izin</h3>
                <p className="text-xs text-gray-500">{selectedAjuan.tanggal}</p>
              </div>
            </div>
            <div className="space-y-4 mb-6">
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">Nama Siswa</label>
                <div className="text-gray-800 font-medium">{selectedAjuan.nama}</div>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">Alasan</label>
                <div className="bg-gray-50 p-3 rounded-lg text-gray-700 italic border border-gray-200">{selectedAjuan.alasan}</div>
              </div>
            </div>
            <div className="flex justify-end gap-3">
              <button 
      ...
      {isEditJamKosongModalOpen && editingJamKosong && (
        <div className="fixed inset-0 bg-black/50 z-[99] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <h3 className="font-bold text-lg mb-4 text-gray-800">✏️ Edit Jam Kosong</h3>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Alasan</label>
                <textarea 
                  className="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none" 
                  rows="3" 
                  value={editingJamKosong.alasan} 
                  onChange={e => setEditingJamKosong({...editingJamKosong, alasan: e.target.value})}
                ></textarea>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Guru Pengganti</label>
                <select 
                  ...
                  onChange={e => setEditingJamKosong({...editingJamKosong, guru_pengganti: e.target.value})}
                >
