...
const validateMaxSize = (file, maxBytes, label) => {
  ...
  throw new Error(
    `File ${label} terlalu besar (${formatFileSize(file.size)}). Maksimal ${maxMB}MB.`
  )
}
...
export default function TugasGuru() {
  ...
  const handleFileSelect = async (files, mode = 'create') => {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        setUploadedFileSizeCreate(formatFileSize(compressedFile.size))
      } else {
        setEditForm((prev) => ({ ...prev, newFile: compressedFile }))
        setUploadedFileSizeEdit(formatFileSize(compressedFile.size))
      }
      pushToast('success', `File siap diupload (${formatFileSize(compressedFile.size)})`)
    } catch (error) {
      console.error('Compression error:', error)
      pushToast('error', `Gagal memproses file: ${error.message}`)
    } finally {
      setCompressionProgress(null)
    }
  }

  const handleEditFileSelect = async (files) => await handleFileSelect(files, 'edit')
  ...
  const openEditTugas = () => {
    ...
    setEditForm({
      ...
      deadline: selectedTugas.deadline ? new Date(selectedTugas.deadline).toISOString().slice(0, 16) : '',
  ...
  const simpanEditTugas = async () => {
    ...
    try {
      ...
      if (editForm.newFile) {
        const safeName = sanitizeFileName(editForm.newFile.name)
        const filePath = `tugas_lampiran/${user.id}/${Date.now()}-${safeName}`
        const { error: uploadError } = await supabase.storage
          .from(ASSIGNMENT_BUCKET)
          .upload(filePath, editForm.newFile, { upsert: false, cacheControl: '3600' })
      ...
      <div className="max-w-full mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-4">
            ...
            <div>
              ...
              {isUploadingFile ? (
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-sm font-medium text-green-800">File siap diupload</div>
                        <div className="text-xs text-green-600">
                          {uploadedFileSizeCreate} • Akan diupload saat simpan
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <FileDropzone
                  onFiles={(files) => handleFileSelect(files, 'create')}
                  accept="*/*"
          ...
          <button
            ...
            onClick={tambahTugas}
            disabled={!kelas || !selectedMapel || !form.judul || !form.deadline || isUploadingFile}
          >
            {isUploadingFile ? (
              <>
              ...
              {isEditingTugas && editForm ? (
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="mt-3">
                      <FileDropzone
                        onFiles={handleEditFileSelect}
                        accept="*/*"
                        maxSize={10 * 1024 * 1024}
                        label="Upload file baru (akan mengganti file lama jika ada)"
                      />
                    </div>
                  </div>
