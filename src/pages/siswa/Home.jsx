...
export default function SHome() {
  ...
  useEffect(() => {
    ...
    const loadAllData = async () => {
      setIsLoading(true)
      try {
        // Load critical data in parallel (pengumuman + absensi for hero section)
        const criticalPromises = [loadPengumuman()]
        if (profile?.kelas) criticalPromises.push(loadAbsensi())

        await Promise.allSettled(criticalPromises)
        setIsLoading(false)

        // Fire all non-critical data loads in parallel (background)
        void Promise.allSettled([
          ...(profile?.kelas ? [loadTugas()] : []),
          loadEskul(),
          loadOrganisasi(),
          loadSertifikat(),
          loadStrukturSekolah()
